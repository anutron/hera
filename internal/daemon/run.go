...
// viewTitle must be non-empty — argus rejects empty titles with HTTP 400,
// crash-looping the daemon (BUG-034). Dropping the visible label is an
// argus-side concern, not achievable by sending an empty string here.
const (
	viewTitle  = "Hera"
	viewHotkey = "ctrl+h"
)
	...
	return []mcp.ToolDefinition{
		{
			...
			InputSchema: map[string]any{
				...
				"properties": map[string]any{
					...
					"prompt":                map[string]any{"type": "string", "description": "(optional) Coordinator's prompt, free-form prose"},
				},
		...
		},
		{
			...
			InputSchema: map[string]any{
				...
				"properties": map[string]any{
					...
					"prompt":       map[string]any{"type": "string", "description": "(optional, attach mode) Role prompt, free-form prose"},
		...
		},
		{
			...
			InputSchema: map[string]any{
				...
				"properties": map[string]any{
					...
					"prompt":       map[string]any{"type": "string", "description": "Full task prompt delivered to the new worker session. An orientation prefix naming the coordinator is prepended automatically. The verbatim prompt is also stored on the role row"},
