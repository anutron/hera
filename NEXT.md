...
- **`hera_spawn_worker` MCP verb – born-bound workers (top v1.x item).** Today a coordinator spawns a worker via raw argus `task_create` + the worker self-joins via `hera_join` in its prompt. That creates a transient "freelancer-then-joins" window and is backwards – the coord knows it's spawning its own worker. The atomic spawn+bind already exists as `internal/view/ops/spawn_worker.go` (`ops.SpawnWorker` – the rail's `w` key): it resolves the coord role's `argus_project`, calls argus `CreateTask` with `meta role=worker`, and inserts the worker role + binding atomically, so the worker is born bound (never a freelancer). The gap: it's only reachable via the operator pressing `w` in the rail – no MCP equivalent for a coordinator agent. Fix: add a 7th hera MCP verb `hera_spawn_worker(cwd, prompt, [role_name], [mission])` that wraps `ops.SpawnWorker`; the worker is born bound under its coord and its prompt just calls `hera_inbox` to read the mission (claim, not attach). Also sidesteps BUG-012.

- **Reliable-messaging full redesign (doorbell-only / email model).** The minimal version shipped in 1.0: a daemon `DeliveryWatcher` re-nudges idle-submit messages whose `read_at` is still null with a non-duplicating `"[hera doorbell] N unread – call hera_inbox"`, plus `read_at` as a receipt. The full v1.x design is the email model: stop injecting the message body into the PTY; inject only a doorbell; the agent pulls the payload via `hera_inbox`; `read_at` is the delivery receipt; re-nudge is idempotent and safe to repeat. Depends on the agent contract (on doorbell → call `hera_inbox`), which was added to the hera skill in 1.0.

- **Role check/archive/resume view-state semantics.** Archive-non-destructive shipped in 1.0 (a bare `task.archived` no longer ends the binding; only `task.deleted`/prune does; resume-from-archived = the resurrect flow). The v1.x refinement is a clean role-level lifecycle: check (park – hidden from active rail, resumable) → archive (hidden, role + inbox persist, resumable) → prune (binding ends; role survives as a durable inbox; needs a fresh agent to re-bind). Make check/archive clean role-level view-state operations with resume wired through; only prune forces re-creation.
