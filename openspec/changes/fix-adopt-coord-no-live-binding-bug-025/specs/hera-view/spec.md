...
When the coordinator is ALREADY nested under another parent, re-parenting SHALL first tear down the prior parent linkage — ending the prior parent's worker link binding (`end_reason="reparented"`) and deleting that worker link role — so the coordinator is never nested under two parents at once. Only a LIVE prior parent linkage is torn down.
