**Design doc:** `openspec/changes/ludwig-v1/design.md`

**Acceptance criteria:** `openspec/changes/ludwig-v1/specs/ludwig-coordination/spec.md` (each `#### Scenario:` is one acceptance criterion)

## 1. Tests

**Depends on:** nothing (this is stage 1).

- [ ] 1.1 Write failing tests for every requirement scenario in `specs/ludwig-coordination/spec.md`. Each `#### Scenario:` becomes one Go test case using table-driven `testify` or stdlib `testing`. Place under `internal/<package>/*_test.go`. Use mock SQLite (in-memory `:memory:` for fast iteration) and `httptest.Server` for argus mocks. Confirm every test FAILS before any implementation (Prove-It pattern).
- [ ] 1.2 Validate the change: `openspec validate ludwig-v1 --strict` MUST pass after the spec text is committed (no implementation needed for this gate).

## 2. Project scaffold

**Depends on:** Stage 1 (tests can be parked under empty packages and remain failing).

- [ ] 2.1 Initialize `go.mod` with `module github.com/anutron/ludwig` and Go version `1.22+`.
- [ ] 2.2 Add Cobra root command at `cmd/ludwig/main.go`. Subcommands: `start`, `stop`, `status`, `resume`, `list`. Each subcommand file under `cmd/ludwig/<verb>.go`. `install` is intentionally absent (deferred follow-up).
- [ ] 2.3 Create package layout: `internal/{config,db,argus,events,mcp,inject,idle,daemon,log}`. Each with a placeholder `doc.go`.
- [ ] 2.4 Add `Makefile` with targets: `build`, `test`, `fmt`, `vet`, `clean`. `build` produces `./bin/ludwig`. `test` runs `go test ./... -race`.
- [ ] 2.5 Add `.gitignore` (Go defaults + `~/.ludwig/` should not be in repo).
...
- [ ] 5.1 Implement `internal/config/config.go`. Default config: argus URL `http://127.0.0.1:7743`, ludwig HTTP listener `127.0.0.1:7744`, state dir `~/.ludwig`, log file `~/.ludwig/ludwig.log`, idle debounce `2s`, MCP heartbeat `5m`.
...
- [ ] 6.3 Implement auto-adopt handler. On a `link.created` event, look up parent's binding via `Bindings.GetLiveByTaskID(parent)`. If parent is bound to a coordinator role AND the new task has `meta:ludwig.role=worker`, fetch the new task's meta for `ludwig.mission` + `ludwig.constraints`, create role + binding rows atomically.
...
- [ ] 8.2 Format body as `[ludwig from <senderRoleName>] <body>`.
...
- [ ] 8.6 Unit-test that the sender prefix is exactly the documented format (`[ludwig from <name>] `).
...
- [ ] 11.2 Resolve sender role via cwd. Resolve recipient: explicit `to` → look up by `(orchestrator_id, role_name)`; absent → default routing (worker/freelance → coordinator; coordinator → `user` pseudo-recipient).
- [ ] 11.3 Insert message row with `delivery_mode=pending`.
- [ ] 11.4 If recipient is `user` pseudo-recipient, set `delivery_mode=user_inbox`, no injection.
- [ ] 11.5 Otherwise, look up recipient's live binding. If none, set `delivery_mode=queued_no_binding`; eventually the next binding for that role can drain. (V1 doesn't implement the drain worker yet; flag as a v1.1 follow-up in the task comment.)
- [ ] 11.6 If recipient has a live binding, call `inject.Inject`. Record returned mode + `delivered_at` on the message row.
- [ ] 11.7 Return `{message_id, recipient_role, delivery_mode}` in MCP response content.
- [ ] 11.8 Unit-test worker → coord, coord → user, worker → explicit other-worker, missing recipient role, idle injection path, busy injection path.

## 12. Handler: ludwig_inbox
...
- [ ] 14.3 Write `meta:ludwig.thread_status=<status>` to the bound argus task via `argus.Client.PutTaskMeta`.
...
- [ ] 15.1 When `Bindings.Create` runs (called by auto-adopt and freelance-join paths), trigger a `meta:ludwig.role=<kind>` write via `argus.Client.PutTaskMeta`.
- [ ] 15.2 When `Bindings.End` runs (called on `task.archived` events), trigger a `meta:ludwig.thread_status=ended` write (or similar) to mark the prior binding done in argus task_meta.
...
- [ ] 16.1 Implement `internal/daemon/run.go`. `Run(ctx context.Context, cfg *config.Config) error`. Order: load token, open DB, build argus client, start event subscriber, start idle tracker subscription, start MCP server, register all five tools, log "ludwig ready" to stderr + log file.
...
- [ ] 16.3 Implement `cmd/ludwig/start.go`. Daemonize via double-fork (or use `daemon.Run` in foreground when `--foreground` is passed). Writes PID to `~/.ludwig/ludwig.pid`.
- [ ] 16.4 Implement `cmd/ludwig/stop.go`. Read PID, send SIGTERM, wait up to 10 seconds for the process to exit.
- [ ] 16.5 Implement `cmd/ludwig/status.go`. Print: PID (if running), uptime, last-seen event id, MCP tool registration status, orchestrator count, role count, live binding count.
- [ ] 16.6 Implement `cmd/ludwig/list.go`. Print orchestrators with their roles and live binding state (live | between incarnations | none).
- [ ] 16.7 Implement `cmd/ludwig/resume.go` as a STUB returning "not implemented in v1; spawn-fresh-incarnation will land in a follow-up change." (Resume depends on argus task_create + worktree setup details that are easier to land alongside the view.)
- [ ] 16.8 Unit-test daemon startup against a mock argus (tools register, event stream connects).
- [ ] 16.9 Smoke test: boot daemon against mock argus, send a fake `task.created` + `link.created` sequence with proper meta, verify ludwig writes a role + binding row.

## 17. Wrap-up

**Depends on:** Stages 1-16.

- [ ] 17.1 Re-run `openspec validate ludwig-v1 --strict`. Must pass.
...
- [ ] 17.4 Final commit + push. Push tag `v0.1.0-headless` (annotated, message: "ludwig v1 headless surface: five MCP tools, role-as-identity, auto-injection, auto-adoption. Plugin view and settings UI deferred.").
