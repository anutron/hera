...
| J — Daemon wire-up | `1779783745520579000` | `argus/hera-view-stage-j-daemon` | restarted (manual) | – | First run transitioned argus status to `<checked>` mid-work, which killed the worker process. Aaron manually restarted. Watch the eventual push carefully — restarted worker may need to reconcile partial state. See "Worker status-flip kills session" follow-up below. |
