...
| J — Daemon wire-up | `1779783745520579000` | `argus/hera-view-stage-j-daemon` | **done** | `ab9294b` (wire) + `ab4928d` (tick), preceded by `a4ebc1c` (octopus merge of B, C, E into J) | First run was killed by the `<checked>` status flip (see follow-up); Aaron restarted. Restart succeeded. Worker did an **octopus merge of stages B + C + E** into its branch before implementing, which made deps' code visible — confirms my "merge deps first" follow-up is the right pattern. Adds `view.Registrar` (mirrors mcp.Registrar shape against argus's plugin-view client) and `view.ProxyManager` (Ensure/Seed/Close over Stage-B subscriptions). New `mcp.Server.Mount(pattern, handler)` substrate API to attach extra routes to the existing MCP listener; `/view` is the first user. Daemon now: starts Registrar with title=Hera, hotkey=ctrl+h, callback=ws://&lt;addr&gt;/view; walks live bindings via new `db.BindingsDAO.ListLive()` to seed ProxyManager; shutdown sequence DELETEs the registration before tearing down MCP. Tests green (registers + unregisters, seeds proxy for live bindings, real websocket.Dial against the mcp listener). |
