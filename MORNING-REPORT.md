...
| K2 — Smoke + session adapter | `1779810843534091000` | `argus/hera-view-stage-k-v2-smoke` | spawned | – | Replacement worker with **expanded scope**: octopus-merge D+F+G into J's branch base, write the per-connection SessionFunc adapter (wsscreen → tview app → KeyRouter), update daemon to pass it into `view.NewServer`, then write the smoke test against the now-real wiring. Hard rule baked in: push immediately on any sign of status flip. |
